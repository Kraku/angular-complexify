...
(function() {
  ...
  angular.module('angular-complexify', [])
    ...
    .provider('Complexify', function() {
      ...
      self.additionalComplexityForCharset = function(str, charset) {
      ...
      self.inBanlist = function(str) {
        if (options.banMode === 'strict') {
          for (var i = 0; i < options.bannedPasswords.length; i++) {
            if (str.toLowerCase().indexOf(options.bannedPasswords[i]) !== -1) {
              return true;
            }
          }
          ...
          return options.bannedPasswords.indexOf(str.toLowerCase()) !== -1;
        }
      }

      self.evaluateSecurity = function(password) {
        var complexity = 0;
        var valid = false;
        ...
        if (!self.inBanlist(password)) {
          ...
          for (var i = CHARSETS.length - 1; i >= 0; i--) {
            complexity += self.additionalComplexityForCharset(password, CHARSETS[i]);
          }
        } else {
          complexity = 1;
        }
      ...
      function(Complexify) {
        return {
          ...
          link: function(scope, elem, attrs) {
            var type = (typeof attrs.type === 'undefined') ? 'percent' : attrs.type;
            var scale = ['Weak', 'Fair', 'Good', 'Strong'];
            ...
            var get = {
              ...
              verbal: function(score) {
                var elem = parseInt(score / 25, 10);

                return scale[elem >= 3 ? 3 : elem];
              }
            };

            scope.$watch('complexify', function(value) {
              var score = Complexify(value).complexity;

              scope.result = get[type](score);
            });
          }
        };
      }
      ...
      return {
        ...
        link: function(scope, elem, attrs, ctrl) {
          ...
          ctrl.$parsers.unshift(function(viewValue) {
            limit = (typeof limit === 'undefined') ? MIN_COMPLEXITY : scope.complexifyValidate;
